...
- Codex 26.900+ (including 26.901.x) is a protected version line: the official build performs whole-file integrity checking on app.asar (any byte change exits the app within seconds). The installer automatically falls back to **locale-only mode** for these versions: it only writes `localeOverride = "zh-CN"` and does not copy/patch/install the entry guard. Chinese UI then depends on the official i18n (enabled by the server once online). Verify is mode-aware (`VERIFY: mode=locale-only`, plus a `locale-note` line). After a locale-only install, ask the user whether the UI is Chinese and report honestly; if English, explain that 26.900+ Chinese is server-gated and domestic networks without OpenAI access will stay English until OpenAI makes the language setting local (tracked via `officialLocaleSince` in `versions.json`).
- For patch-mode installs (Codex < 26.900), offer/suggest freezing Store auto-updates (`-Action freeze` / menu item 7) so the Store cannot upgrade the app to a protected version and break localization.

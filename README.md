# Codex 一键汉化（Codex Desktop zh-CN Agent Installer）v1.3.3
...
1. 下载 Release 中的 `codex-zh-cn-agent-v1.3.3.zip` 并解压；
...
- **GitHub 打不开 / 下载不了仓库？** agent 会自动按「GitHub 直连 → 备用域名 → 镜像站」的顺序尝试，全程不会改动你的网络设置；如果全部失败，会明确告诉你改用离线包：从 Release 下载 `codex-zh-cn-agent-v1.3.3.zip`（可以请朋友帮忙下载后传给你），解压后双击「安装汉化.bat」即可，全程不需要访问 GitHub。

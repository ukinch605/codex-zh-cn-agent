# Codex 一键汉化（Codex Desktop zh-CN Agent Installer）v1.2
...
Codex 会按 `AGENTS.md` 的流程自动完成：拉取仓库 → 检测版本 → 安装（弹一次 UAC，点「是」）→ 自动重启为中文版（几秒内完成，无后台驻留）→ 验证并汇报。
...
| Codex 版本 | 测试日期 | 状态 | 备注 |
| --- | --- | --- | --- |
| 26.803.5235.0 | 2026-08-08 | ✅ 通过 | 本仓库 v1.2 收录 |
...
- **打开还是英文？** 安装完成后当前窗口会自动以中文版重启（几秒过渡，不驻留后台）。日常使用请通过桌面快捷方式「Codex 汉化版」或「启动汉化版.bat」打开；如果从开始菜单/任务栏的原版入口打开，打开的是英文原版（与所有同类汉化工具一致）。若仍显示英文，先彻底关闭 Codex（任务栏右键退出），再双击「启动汉化版.bat」，仍不行就重跑「安装汉化.bat」。
...
6. 安装完成后的重启由监督脚本完成：关闭旧进程 → 以普通权限启动汉化副本 → 验证稳定后立即退出（不驻留），全过程写入日志与 `launch-result.json`。
